...
pub fn firefox_based(
  ...
  .to_string();
    ...
    if let Ok(session_cookies) =
        get_session_cookies_lz4(domains.to_owned(), parent_path.to_owned())
    {
...
pub fn get_session_cookies_lz4(
    ...
    let json: Value = serde_json::from_str(&plain)?;
    let cookies_json =
        json.get("cookies").ok_or(anyhow!("no cookies in json"))?;
...
pub fn get_default_profile(profiles_path: &Path) -> Result<String> {
    ...
    let installs: Vec<_> = conf
        .iter()
        .filter(|(name_option, _)| {
            name_option.unwrap_or_default().starts_with("Install")
        })
        .collect();
    if !installs.is_empty() {
        ...
        let profiles: Vec<_> = conf
            .iter()
            .filter(|(name_option, _)| {
                name_option.unwrap_or_default().starts_with("Profile")
            })
            .collect();
