...
pub fn get_keys(key64: &str) -> Result<Vec<Vec<u8>>> {
    ...
    let mut system_decrypted =
        decrypt(&mut BASE64_STANDARD.decode(key64)?, true)?;
    let user_decrypted = decrypt(&mut system_decrypted, false)?;
    let key = &user_decrypted[user_decrypted.len() - 61..];

    // Most chrome browsers can use the system->user decrypted key directly
    // (last 32 bytes)
    keys.push(key[key.len() - 32..].to_vec());

    // Chrome also decrypt the decrypted key with hardcoded AES key from
