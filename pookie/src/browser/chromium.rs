...
pub fn chromium_based(
    ...
    let key_dict: serde_json::Value = serde_json::from_str(content.as_str())
        .context("Can't read json file")?;
...
fn get_keys(config: &Browser) -> Result<Vec<Vec<u8>>> {
    ...
    if let Ok(passwords) = crate::linux::get_passwords(
        &config.unix_crypt_name.clone().unwrap_or("".to_owned()),
    ) {
...
fn get_keys(config: &Browser) -> Result<Vec<Vec<u8>>> {
    ...
    let password = macos::get_osx_keychain_password(&key_service, &key_user)
...
fn decrypt_encrypted_value(
    ...
    if !value.is_empty()
        || !(key_type == b"v11" || key_type == b"v10" || key_type == b"v20")
    {
            ...
            match decoded {
                ...
                Err(_) => {
                    ...
                    let decoded = String::from_utf8(plaintext[32..].to_vec())
                        .unwrap_or_else(|_| {
                            log::warn!(
                                "Error decoding from index 32 with UTF-8"
                            );
                            "".into()
                        });
                    return Ok(decoded);
...
fn unlock_file(mut path: PathBuf) -> Result<PathBuf> {
    ...
    if privilege::user::privileged() {
        ...
        {
            let result = windows::shadow_copy::shadow_copy(
                path.clone(),
                temp_dir.clone().to_path_buf(),
            );
...
fn query_cookies(
    ...
    while let Some(row) = rows.next()? {
        ...
        let decrypted_value =
            decrypt_encrypted_value(value, &encrypted_value, keys.to_owned())?;
